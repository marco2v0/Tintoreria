...
def impresion(nota):
	...
	for row in nota.detalle.all():
		v_articulo = row.articulo.descripcion_corta[0:8]
		v_servicio = row.servicio.nombre_corto[0:8]
	...
	p.text("----------------------------------------------\n")
	p.text("Descuento:".ljust(36,' ')+("$"+str(nota.descuento)).rjust(9,' ')+"\n")
	p.text("----------------------------------------------\n")
	p.text("Total a Pagar:".ljust(36,' ')+("$"+str(nota.total_apagar)).rjust(9,' ')+"\n")
	p.text("----------------------------------------------\n")
	p.text("A cuenta: $"+str(nota.pagado)+"\n")
	p.text("Debe: $"+str(nota.total_apagar - nota.pagado)+"\n")
